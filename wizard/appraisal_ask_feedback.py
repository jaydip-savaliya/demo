...
class AppraisalAskFeedback(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        appraisal = self.env['appraisal.appraisal'].browse(result.get('appraisal_id'))
        if 'survey_template_id' in fields and appraisal and not result.get('survey_template_id'):
            result[
                'survey_template_id'] = appraisal.department_id.appraisal_survey_template_id.id or appraisal.company_id.appraisal_survey_template_id.id
        return result

    appraisal_id = fields.Many2one('appraisal.appraisal', default=lambda self: self.env.context.get('active_id', None))
    employee_id = fields.Many2one(related='appraisal_id.employee_id', string='Appraisal Employee')
    template_id = fields.Many2one(
        default=lambda self: self.env.ref('hr_appraisal_survey.mail_template_appraisal_ask_feedback',
    ...
    deadline = fields.Date(string="Answer Deadline", required=True, compute='_compute_deadline', store=True,
                           readonly=False)
    ...
    def _compute_subject(self):
        for wizard in self.filtered('employee_id'):
            if wizard.template_id:
                wizard.subject = \
                self.sudo()._render_template(wizard.template_id.subject, 'appraisal.appraisal', wizard.appraisal_id.ids,
                                             post_process=True)[wizard.appraisal_id.id]
    ...
    def _send_mail(self, answer):
        ...
        try:
            ...
            _logger.warning(
                'QWeb template mail.mail_notification_light not found when sending appraisal feedback mails. Sending without layouting.')
        else:
            template_ctx = {
                'message': self.env['mail.message'].sudo().new(
                    dict(body=mail_values['body_html'], record_name=self.survey_template_id.title)),
    ...
    def action_send(self):
        ...
        for employee in self.employee_ids.filtered(
                lambda e: e.user_id.has_group('aspl_hr_appraisal.group_hr_appraisal_manager')):
            ...
            self.appraisal_id.with_context(mail_activity_quick_update=True).activity_schedule(
                ...
                note=_(
                    'An appraisal feedback was requested. Please take time to fill the <a href="%s" target="_blank">survey</a>') %
