...
class AppraisalAskFeedback(models.TransientModel):
    ...
    def _compute_subject(self):
        for wizard in self.filtered('employee_id'):
            if wizard.template_id:
                wizard.subject = self.sudo()._render_template(wizard.template_id.subject, 'appraisal.appraisal', wizard.appraisal_id.ids, post_process=True)[wizard.appraisal_id.id]
    ...
    def action_send(self):
        ...
        for employee in self.employee_ids.filtered(lambda e: e.user_id.has_group('aspl_hr_appraisal.group_hr_appraisal_manager')):
